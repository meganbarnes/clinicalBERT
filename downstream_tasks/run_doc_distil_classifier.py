...
from transformers import DistilBertForSequenceClassification, DistilBertConfig, WEIGHTS_NAME, CONFIG_NAME
from transformers import DistilBertTokenizer
from transformers import AdamW, get_linear_schedule_with_warmup
from pytorch_pretrained_bert.optimization import warmup_linear

from doc_distilbert import DocDistilBertForSequenceClassification
...
def convert_documents_to_features(examples, label_list, max_seq_length, tokenizer):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for seq_index, i in enumerate(range(0, len(tokens_a), (max_input_length-2))):
            raw_tokens = tokens_a[i:i+(max_input_length-2)]
...
def main():
    ...
    if args.fp16:
        ...
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=float(num_train_optimization_steps)*args.warmup_proportion, num_training_steps=num_train_optimization_steps)
    ...
    tr_loss = 0
    if args.do_train:
        train_features = convert_documents_to_features(
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss, logits, other = model(input_ids=input_ids, attention_mask=input_mask, labels=label_ids)
    ...
    if args.do_train:
        ...
        model = DistilBertForSequenceClassification.from_pretrained(args.output_dir) #, num_labels=num_labels)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_features = convert_documents_to_features(
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                tmp_eval_loss, logits, other = model(input_ids=input_ids, attention_mask=input_mask, labels=label_ids)
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            ...
            test_features = convert_documents_to_features(
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(test_dataloader, desc="Testing"):
                ...
                with torch.no_grad():
                    ...
                    tmp_test_loss, logits, other = model(input_ids=input_ids, attention_mask=input_mask, labels=label_ids)
                    #logits = model(input_ids, segment_ids, input_mask)
